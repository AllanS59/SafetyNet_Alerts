...
public class SpecificQueriesControllerTest {
	...
	public void testGetPersonsByStation () throws Exception   {
      	mockMvc.perform(get("/firestation?stationNumber=3")).andExpect(status().isOk()).andExpect(jsonPath("numberChildren", is(3)));
	}
	...
	public void testPersonsInfosByName () throws Exception   {
      	mockMvc.perform(get("/personInfo?firstName=Jonanathan&lastName=Marrack")).andExpect(status().isOk()).andExpect(jsonPath("firstName", is("Jonanathan")));
	}
